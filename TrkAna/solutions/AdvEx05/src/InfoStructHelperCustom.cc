...
namespace mu2e {
  void InfoStructHelperCustom::fillHitCount(StrawHitFlagCollection const& shfC, HitCount& hitcount) {
  ...
  void InfoStructHelperCustom::fillHitCount(RecoCount const& nrec, HitCount& hitcount) {
  ...
  void InfoStructHelperCustom::fillTrkInfo(const KalSeed& kseed,TrkInfo& trkinfo) {
  ...
  void InfoStructHelperCustom::fillTrkFitInfo(const KalSeed& kseed,TrkFitInfo& trkfitinfo, const XYZVec& pos) {
    const auto& ksegIter = kseed.nearestSegment(pos);
    if (ksegIter == kseed.segments().end()) {
      cet::exception("InfoStructHelperCustom") << "Couldn't find KalSegment that includes pos = " << pos;
    }
  ...
  void InfoStructHelperCustom::fillTrkInfoHits(const KalSeed& kseed, TrkInfo& trkinfo) {
  ...
  void InfoStructHelperCustom::fillTrkInfoStraws(const KalSeed& kseed, TrkInfo& trkinfo) {
  ...
  void InfoStructHelperCustom::fillHitInfo(const KalSeed& kseed, std::vector<TrkStrawHitInfo>& tshinfos ) {
    tshinfos.clear();
    // loop over hits
  ...
  void InfoStructHelperCustom::fillMatInfo(const KalSeed& kseed, std::vector<TrkStrawMatInfo>& tminfos ) {
    tminfos.clear();
  ...
  void InfoStructHelperCustom::fillCaloHitInfo(const KalSeed& kseed, TrkCaloHitInfo& tchinfo) {
    if (kseed.hasCaloCluster()) {
  ...
  void InfoStructHelperCustom::fillTrkQualInfo(const TrkQual& tqual, TrkQualInfo& trkqualInfo) {
  ...
  void InfoStructHelperCustom::fillHelixInfo(const KalSeed& kseed, HelixInfo& hinfo) {
  ...
  void InfoStructHelperCustom::fillTrkPIDInfo(const TrkCaloHitPID& tchp, const KalSeed& kseed, TrkPIDInfo& trkpidInfo) {
    mu2e::GeomHandle<mu2e::Calorimeter> calo;
  ...
  void InfoStructHelperCustom::fillEWMInfo(const EventWindowMarker& ewm, EWMInfo& info) {
    info._tOffset = ewm._timeOffset;
  }
}
