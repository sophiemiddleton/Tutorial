...
#include "Offline/TrkDiag/inc/InfoMCStructHelper.hh"
#include "Offline/RecoDataProducts/inc/RecoQual.hh"
#include "Offline/TrkDiag/inc/RecoQualInfo.hh"
...
// Need this for the BaBar headers.
  ...
  class TrackAnalysisRecoCustom : public art::EDAnalyzer {
  ...
  public:
    ...
    struct Config {
      ...
      fhicl::Atom<art::InputTag> ewmTag{Name("EventWindowMarkerTag"), Comment("Tag for EventWindowMarker"), art::InputTag()};
    };
    typedef art::EDAnalyzer::Table<Config> Parameters;

    explicit TrackAnalysisRecoCustom(const Parameters& conf);
    virtual ~TrackAnalysisRecoCustom() { }
  ...
  private:
    ...
    HelixInfo _hinfo;
    // struct helpers
    InfoStructHelperCustom _infoStructHelper;
  ...
  TrackAnalysisRecoCustom::TrackAnalysisRecoCustom(const Parameters& conf):
    art::EDAnalyzer(conf),
    _conf(conf()),
  ...
  void TrackAnalysisRecoCustom::beginJob( ){
...
// helix info
  ...
  void TrackAnalysisRecoCustom::beginSubRun(const art::SubRun & subrun ) {
  ...
  void TrackAnalysisRecoCustom::analyze(const art::Event& event) {
...
//	find the best CRV match (closest in time)
  ...
  size_t TrackAnalysisRecoCustom::findSupplementTrack(KalSeedCollection const& kcol,const KalSeed& candidate, bool sameColl) {
  ...
  void TrackAnalysisRecoCustom::fillEventInfo( const art::Event& event) {
  ...
  void TrackAnalysisRecoCustom::fillTriggerBits(const art::Event& event,std::string const& process) {
  ...
  void TrackAnalysisRecoCustom::fillAllInfos(const art::Handle<KalSeedCollection>& ksch, size_t i_branch, size_t i_kseed) {
...
// fill MC info associated with this track
  ...
  template <typename T, typename TI>
  std::vector<art::Handle<T> >  TrackAnalysisRecoCustom::createSpecialBranch(const art::Event& event, const std::string& branchname, 
								       std::vector<art::Handle<T> >& handles, TI& infostruct, const std::string& selection) {
  ...
  void TrackAnalysisRecoCustom::resetBranches() {
    // reset structs
    _einfo.reset();
    _hcnt.reset();
    _tcnt.reset();
...
using mu2e::TrackAnalysisRecoCustom;
DEFINE_ART_MODULE(TrackAnalysisRecoCustom);
